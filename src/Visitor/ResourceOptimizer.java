...
public class ResourceOptimizer implements SystemVisitor {
    ...
    public void visitCPU(CPU cpu) {
        if (cpu.getUsagePercentage() > 80) {
            ReportManager.getInstance().addLog("WARNING: High CPU usage detected! (" + cpu.getUsagePercentage() + "%)");
        }
    }
    ...
    public void visitMemory(Memory memory) {
        if (memory.getUsedMemoryMB() > 8000) {
            ReportManager.getInstance().addLog("WARNING: High memory consumption! (" + memory.getUsedMemoryMB() + " MB)");
        }
    }
    ...
    public void visitDisk(Disk disk) {
        if (disk.getUsagePercentage() > 90) {
            ReportManager.getInstance().addLog("WARNING: Disk capacity critical! (" + disk.getUsagePercentage() + "%)");
        }
    }
